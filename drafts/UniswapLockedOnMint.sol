...
contract UniswapLockedOnMint{
    ...
    LaunchpadToken private _token;


    IUniswapV2Router02 private _uniRouter;

    constructor(IUniswapV2Router02 uniRouter, uint256 releaseTime, address owner, uint256 price) public{
    ...
    function setupToken(LaunchpadToken token) public {
    ...
    *
    ...
    function addLiquidityOnMint() public {
        ...
        require(address(token) != address(0)), "UniswapLockedOnMint: Token can not be zero");
        require(etherBalance > 0, "UniswapLockedOnMint: no ether to add liquidity");
        require(msg.sender == address(_deployer), "UniswapLockedOnMint: Only deployer call this function");
